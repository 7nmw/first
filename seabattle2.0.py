...
# размещение корабля
    def add_ship(self, ship):
       for d in ship.dots:
           if self.out(d) or d in self.busy: # если точка не выходит и не занята еще
               raise BoardWrongShipException()
       for d in ship.dots:
           self.field[d.x][d.y] = "■" #ставим на месте выстрела ""
           self.busy.append(d) # запишем точки
       self.ships.append(ship) # добавляем список собсвтенных кораблей
...
class User(Player):
    def ask(self):
        while True:
            ...
            return Dot(x-1, y-1) # вычетаем 1 так как индексация с 0
 ...
 #игровой цикл
    def loop(self):
        ...
        while True:
            ...
            print(self.ai.board)
            print("-" * 20)
            if num % 2 == 0: # если номер хода четый, то ходит пользователь
            ...
            if self.ai.board.defeat(): #кол-во пораженных кораблей
                print("-" * 20)
